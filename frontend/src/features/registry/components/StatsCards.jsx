...
const DECISION_CARDS = [
  ...
  { key: 'total_amount_pending', label: 'Εκκρεμή (€)', icon: Euro, color: 'text-orange-600', bg: 'bg-orange-50', format: 'currency' },
  { key: 'total_amount_paid', label: 'Εισπράχθηκαν (€)', icon: Euro, color: 'text-green-600', bg: 'bg-green-50', format: 'currency' },
];
    ...
    <div className="space-y-4">
      ...
      {decisionStats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {DECISION_CARDS.map(({ key, label, icon: Icon, color, bg, link, format }) => {
            const content = (
